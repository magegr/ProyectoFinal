...
@Component({
  selector: 'app-gafa-graduada',
  imports: [Carrusel, Cards, InfoProduct],
  templateUrl: './gafa-graduada.html',
  styleUrl: './gafa-graduada.css',
})
export class GafaGraduada {
 ...
 public cdr = inject(ChangeDetectorRef) //servicio para actualizar
  ...
  public getProducts(){
   ...
   this.data.getProduct().subscribe(response =>{
        this.info=response.filter(product=>product.type==='graduadas')
